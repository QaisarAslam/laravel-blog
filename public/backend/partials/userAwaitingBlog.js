$(document).ready(function() {
	var table = $('#userAwaitingBlogs').DataTable({ //implement data on #awaiting id="awaiting" in awaiting.blade.php
		processing: true,
		serverSide: true,
		reponsive: true,
		autowidth: true,
		...
		"ajax" : {
			'type': 'POST',
			'url': baseUrl+'/user/getUserAwaitingBlogs',
			'data' : {
		...
		},
		{
			"render" : function (data, type, row, meta)
			{
				return `<a href="${baseUrl}/user/editBlog/${row.id}" class="btn btn-primary btn-sm editTag" id="${row.id}"><i class='fas fa-pencil-alt'</i></a>`;
			},
			"targets" : 7
		},
		{
		...
		$(document).on('click', '.deleteBlog', function(e){ //Ep_23(04:32)
			e.preventDefault();
			var id = $(this).attr('id');
			alert(id);
		})
